...
using MD.Framework.Utility.Core.Helpers;
	...
	public class Criteria<TEntity> where TEntity : class
	{
		...
		public Criteria<TEntity> Or(Criteria<TEntity> critaria)
		{
			...
			if (_entityType != critaria._entityType)
				throw new Exception($"critaria must be from '{_entityType.GetTypeInfo().Assembly.FullName}' type");
		...
		private static Expression GetConditionExpression(ConditionTree conditionForConvert, Type parameterExpressionType, ParameterExpression parameterExpression)
		{
			...
			if (conditionForConvert.OperationType == OperatorEnum.None
				&& string.Equals(conditionForConvert.SerializedValue, ((int)TrueFalseEnum.True).ToString(CultureInfo.InvariantCulture), StringComparison.OrdinalIgnoreCase))
			{
			...
			// False
			if (conditionForConvert.OperationType == OperatorEnum.None
				&& string.Equals(conditionForConvert.SerializedValue, ((int)TrueFalseEnum.False).ToString(CultureInfo.InvariantCulture), StringComparison.OrdinalIgnoreCase ))
			{
				var constantExpression = Expression.Constant(1, typeof(int));
		...
		private static object ChangeValueType(Type targetPropertyType, object value)
		{
			...
			{
				if (PersianDateTime.Core.PersianDateTime.IsChristianDate(propertyValueInString))
					propertyValue = DateTime.Parse(propertyValueInString);
				else
					propertyValue = PersianDateTime.Core.PersianDateTime.Parse(propertyValueInString).ToDateTime();
			}
